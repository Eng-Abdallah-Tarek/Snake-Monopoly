...
Output::Output()
{
	...
	UI.CellHeight = (UI.height - UI.ToolBarHeight - UI.StatusBarHeight) / NumVerticalCells; //55

	// Pen Colors of messages of status bar and players' info
	UI.MsgColor = BLACK ;//DARKRED
	UI.PlayerInfoColor =DARKBLUE ;//DARKSLATEBLUE

	// Background Colors of toolbar and statusbar 
	UI.ToolBarColor = WHITE;
	UI.StatusBarColor = color(255,255,230);//LIGHTGRAY

	// Line Colors of the borders of each cell
	UI.GridLineColor = WHITE;

	// Cell Color if Empty & Cell Number Font & Color
	UI.CellColor_NoCard = DARKCYAN; //LIGHTSLATEBLUE
	UI.CellNumFont = 13;
	UI.CellNumColor = UI.GridLineColor;						//doesn't mean anything should be deleted because it is made WHITE in line 45

	// Cell Color if Has Card & CARD Number Font & Color
	UI.CellColor_HasCard = DARKGREEN;
	UI.CardNumFont = 30;
	UI.CardNumColor = WHITE;

	// Ladder Line Width and Color
	UI.LadderlineWidth = 6;
	UI.LadderColor = GOLD; //DARKSLATEBLUE

	// The X and Y Offsets of the Space BEFORE Drawing the Ladder (offset from the start X and Y of the Cell)
	UI.LadderXOffset = (UI.CellWidth - 2 * UI.LadderlineWidth) / 5;		//19.6 == 19
	UI.LadderYOffset = UI.CellHeight / 2;								//27.5 == 27

	// Snake Line Width and Color
	UI.SnakelineWidth = 6;
	UI.SnakeColor = FIREBRICK; // BROWN color(240,30,20)
	...
	pWind = CreateWind(UI.width +15 , UI.height, UI.wx, UI.wy);			//WHY +15 ?
...
{
	//make sure this is a valid cell before drawing. if not , don't draw!
	if (cellPos.VCell() == -1 || cellPos.HCell() == -1)
		return;
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_GAME_OBJECT]="images\\Delete_Object.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\input_roll.jpg";
...
//////////////////////////////////////////////////////////////////////////////////////////
void Output::ClearToolBarForInfo(int w, int h, int x, int y) const
{
	pWind->SetPen(UI.ToolBarColor, 1);
	pWind->SetBrush(UI.ToolBarColor);
	pWind->DrawRectangle(w - 1, h - 1, x + 1, y + 1);
}
//////////////////////////////////////////////////////////////////////////////////////////
...
//TRY REMOVING -W AND -H AND SEE WHAT HAPPENS ?
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	///TODO: Validate the playerNum and the cell to draw in, if not valid return
	if (playerNum >= MaxPlayerCount  || playerNum<0 ||cellPos.HCell()==-1 || cellPos.VCell() == -1)								//check;
		return;
	...
	int radius = UI.CellWidth / 14; // proportional to cell width  //7.857==7

	// Calculate the horizontal space before drawing players circles (space from the left border of the cell)
	int ySpace = UI.CellHeight / 6; // proportional to cell height		//9.16==9
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	///TODO: Validate the Cell Position (Must be Vertical Cells AND toCell is above fromCell, otherwise, Do NOT draw
			// Validation that the cell is valid 
	if (fromCell.HCell() != toCell.HCell() || (fromCell.HCell()==-1 || fromCell.VCell()==-1 || toCell.HCell() ==-1 || toCell.VCell()==-1 ) || (fromCell.VCell() < toCell.VCell()) )
		return;												/*check!!*/
...
///TODO: Set pen color and width using the appropriate variables of UI_Info object (UI)
	...
	int linex1offset = x12;
	int linex2offset = x34 ;
	int lineyoffset = fromStartY + UI.LadderlineWidth/4 ;	//because 4 is the most perfect
	int numberoflines = fromCell.VCell() - toCell.VCell();

	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);		//Do we have to setpen every time ???!

	for (int i = 1; i <= numberoflines; i++)
	{
		pWind->DrawLine(linex1offset, lineyoffset, linex2offset, lineyoffset);
		lineyoffset -= UI.CellHeight;
	}
}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int x12 = cellStartX + UI.LadderXOffset / 2; // same for the start and end point (vertical)	**** //it is better to be UI>LadderXOffset ONLY
	...
	//																							2
	///TODO: Set the coordinates of the 4 points of the Polygon								1		3		
	//       Check the snakes drawn in the project document and draw it the same way			4					
	int xco= cellStartX + UI.SnakelineWidth / 3.1;	//that conversion to float make it good !
	int xarr[4] =
	{
		xco,
		xco + xChange ,
